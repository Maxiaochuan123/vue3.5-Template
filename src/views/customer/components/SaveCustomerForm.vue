...
const editData = inject<Ref<Partial<CustomerSaveForm>>>('editData')!
const customerDetail = inject<Ref<CustomerDetail | null>>('customerDetail')!
...
// 计算属性：是否显示财务信息
const showFinanceInfo = computed(() => customerDetail.value?.status !== 2)

// 计算属性：是否显示不通过理由
const showRejectReason = computed(() => customerDetail.value?.status === 1)
        ...
        <div class="info-row">
          <div class="info-section">
            ...
            <NDescriptions :column="1" :label-placement="'left'" :label-align="'right'">
              <NDescriptionsItem label="联系人">{{ customerDetail?.userName }}</NDescriptionsItem>
              <NDescriptionsItem label="性别">{{ getOptionLabel(genderOptions, customerDetail?.gender) }}</NDescriptionsItem>
              <NDescriptionsItem label="手机号">{{ customerDetail?.mobile }}</NDescriptionsItem>
              <NDescriptionsItem label="归属地区">{{ customerDetail?.address }}</NDescriptionsItem>
              <NDescriptionsItem label="申请注册时间">{{ customerDetail?.createTime }}</NDescriptionsItem>
            </NDescriptions>
          </div>
          ...
          <div v-if="showFinanceInfo" class="info-section">
            ...
            <NDescriptions :column="1" :label-placement="'left'" :label-align="'right'">
              <NDescriptionsItem label="累计充值">{{ customerDetail?.allRecharge }}</NDescriptionsItem>
              <NDescriptionsItem label="充值本金">{{ customerDetail?.rechargePrincipal }}</NDescriptionsItem>
              <NDescriptionsItem label="赠送金额">{{ customerDetail?.rechargeGift }}</NDescriptionsItem>
          ...
          <div v-if="showFinanceInfo" class="info-section">
            ...
            <NDescriptions :column="1" :label-placement="'left'" :label-align="'right'">
              <NDescriptionsItem label="当前本金">{{ customerDetail?.principal }}</NDescriptionsItem>
              <NDescriptionsItem label="当前赠金">{{ customerDetail?.gift }}</NDescriptionsItem>
        ...
        <div class="info-row">
          <div class="info-section">
            ...
            <NDescriptions :column="3" :label-placement="'left'" :label-align="'right'">
              <NDescriptionsItem label="审核人">{{ customerDetail?.auditUser }}</NDescriptionsItem>
              <NDescriptionsItem label="审核结果">{{ getOptionLabel(auditStatusOptions, customerDetail?.status) }}</NDescriptionsItem>
              <NDescriptionsItem label="审核时间">{{ customerDetail?.auditTime }}</NDescriptionsItem>
              <NDescriptionsItem v-if="showRejectReason" label="不通过理由">{{ customerDetail?.auditContent }}</NDescriptionsItem>
            </NDescriptions>
          </div>
        </div>
      </NCard>
